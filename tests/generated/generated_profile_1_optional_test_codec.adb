...
package body Generated_Profile_1_Optional_Test_Codec is
   ...
   Number_Tag : constant Profile.Field_Tag := 1;

   Number_First_Binding_Check : constant Value :=
     (Number => Interfaces.Integer_64'First,
      Has_Number => False);
   pragma Unreferenced (Number_First_Binding_Check);

   Number_Last_Binding_Check : constant Value := (Number => Interfaces.Integer_64'Last, Has_Number => False);
   pragma Unreferenced (Number_Last_Binding_Check);
   ...
   begin
      Size := 0;
      if Item.Has_Number and then (Item.Number < -1 or else Item.Number > 1) then
         Status := Flyology_Wire.Codecs.Invalid_Value;
         return;
      end if;

      if Item.Has_Number then
         Profile.Measure_Field
           (Number_Tag,
            Flyology_Wire.Byte_Count (Profile.Unsigned_Size (Profile.ZigZag_Encode (Item.Number))),
            Field_Size,
            Arithmetic);
   ...
   begin
      ...
      if Item.Has_Number then
         ...
         Profile.Write_Signed (Output, Nested, Item.Number, Write_Result);
   ...
   is
      ...
      Candidate     : Value := (Number => -1, Has_Number => False);
      Raw_Number : Interfaces.Integer_64;
   begin
      Item := (Number => -1, Has_Number => False);
