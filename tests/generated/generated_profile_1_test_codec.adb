...
package body Generated_Profile_1_Test_Codec is
   ...
   Ignored_Writer_Tag_3 : constant Profile.Field_Tag := 3;

   Accepted_Writers : constant Compatibility.Schema_Identity_Array :=
     [1 => Accepted_Writer_1_Schema, 2 => Accepted_Writer_2_Schema];

   Code_First_Binding_Check : constant Value := (Code => Interfaces.Unsigned_64'First, Enabled => False);
   pragma Unreferenced (Code_First_Binding_Check);

   Code_Last_Binding_Check : constant Value := (Code => Interfaces.Unsigned_64'Last, Enabled => False);
   ...
   is
      ...
      Candidate     : Value := (Code => 1, Enabled => False);
   ...
   begin
      ...
      while not Profile.At_End (Reader) loop
         ...
         if Tag = Code_Tag then
            ...
            if Writer = Accepted_Writer_1_Schema then
            ...
            Seen_Enabled := True;
         elsif Tag = Ignored_Writer_Tag_3
           and then (Writer = Accepted_Writer_2_Schema)
         then
            if Writer = Accepted_Writer_2_Schema then
               if Flyology_Wire.Byte_Count (Region.Length) > 1 then
                  Status := Flyology_Wire.Codecs.Invalid_Value;
                  return;
               end if;
            end if;
      ...
      if not Seen_Enabled
        and then Writer = Accepted_Writer_1_Schema
      then
         Candidate.Enabled := False;
         Seen_Enabled := True;
      end if;

      if not Seen_Code or else not Seen_Enabled then
         Status := Flyology_Wire.Codecs.Invalid_Value;
      else
         Item := Candidate;
...
end Generated_Profile_1_Test_Codec;
