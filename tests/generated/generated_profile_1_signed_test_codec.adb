...
package body Generated_Profile_1_Signed_Test_Codec is
   ...
   Count_Tag   : constant Profile.Field_Tag := 1;
   Enabled_Tag : constant Profile.Field_Tag := 2;

   Count_First_Binding_Check : constant Value := (Count => Interfaces.Integer_64'First, Enabled => False);
   pragma Unreferenced (Count_First_Binding_Check);

   Count_Last_Binding_Check : constant Value := (Count => Interfaces.Integer_64'Last, Enabled => False);
   pragma Unreferenced (Count_Last_Binding_Check);
   ...
   begin
      Size := 0;
      if Item.Count < -4 or else Item.Count > 7 then
      ...
      Profile.Measure_Field
        (Count_Tag,
         Flyology_Wire.Byte_Count (Profile.Unsigned_Size (Profile.ZigZag_Encode (Item.Count))),
         Field_Size,
         Arithmetic);
   ...
   begin
      ...
      Profile.Write_Signed (Output, Nested, Item.Count, Write_Result);
   ...
   is
      ...
      Candidate     : Value := (Count => -4, Enabled => False);
      Raw_Count : Interfaces.Integer_64;
      Seen_Count   : Boolean := False;
      Seen_Enabled : Boolean := False;
   begin
      Item := (Count => -4, Enabled => False);
      ...
      if not Seen_Count or else not Seen_Enabled then
         Status := Flyology_Wire.Codecs.Invalid_Value;
      else
         Item := Candidate;
