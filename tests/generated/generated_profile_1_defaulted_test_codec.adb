...
package body Generated_Profile_1_Defaulted_Test_Codec is
   ...
   Code_Tag    : constant Profile.Field_Tag := 2;

   Code_First_Binding_Check : constant Value := (Enabled => False, Code => Interfaces.Unsigned_64'First);
   pragma Unreferenced (Code_First_Binding_Check);

   Code_Last_Binding_Check : constant Value := (Enabled => False, Code => Interfaces.Unsigned_64'Last);
   ...
   begin
      ...
      if Arithmetic = Sizes.Computed and then Item.Code /= 0 then
         Profile.Measure_Field
           (Code_Tag,
